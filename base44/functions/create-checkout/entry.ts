...
Deno.serve(async (req: Request) => {
  try {
    ...
    // ===== APP-SPECIFIC =====
    // Budgey Pro — two purchase paths, both resolved SERVER-SIDE so a buyer can never tamper
    // with the price. The client sends only `purchaseType` ("installment" | "lifetime").
    //   - installment: $5 one-time consumable. Each purchase adds $5 to the buyer's total_paid
    //                  and grants 30 days of Pro. When total_paid >= $20 the webhook flips
    //                  is_permanent = true (Pro forever, never pay again).
    //   - lifetime:    $20 one-time, permanent immediately.
    const purchaseType = String(body.purchaseType ?? "installment").toLowerCase();
    const isLifetime = purchaseType === "lifetime";
    const productId = isLifetime ? "budgie-premium-lifetime" : "budgey_5";
    // Fixed-entitlement product: hard-code quantity 1 and ignore any buyer-supplied quantity.
    const quantity = 1;
    const productName = isLifetime ? "Budgey Pro (Lifetime)" : "Budgey Pro Installment ($5)";
    const price = isLifetime ? "20.00" : "5.00";  // authoritative price (major units), server-side
    const currency = "USD";
    // Both paths are one-time (non-subscription) purchases — no subscriptionInfo.
    ...
    const thankYouPath = "/premium-success";
    const postFlowPath = "/";
    // ===== END APP-SPECIFIC =====
