...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant: Budgey Pro has two paths — installment ($5 one-time consumable) or lifetime ($20 one-time).
  // Create/update a PremiumEntitlement row keyed on the buyer's app user id. The client verifies
  // Pro status by reading that row (RLS lets the owner read their own row; only the service role
  // can create it, so a buyer can't forge Pro). For installment: add $5 to total_paid, extend
  // pro_until by 30 days, and flip is_permanent once total_paid >= $20. For lifetime: is_permanent
  // is true immediately. A duplicate ORDER_APPROVED for the same checkout is blocked by the
  // terminal-state check above. Runs before the "paid" flip below so a failure leaves status
  // "pending" and Wix retries the grant.
  const isLifetimePurchase = purchase.productId === "budgie-premium-lifetime";
  const isInstallmentPurchase = purchase.productId === "budgey_5";
  if (purchase.appUserId) {
    const existing = await db.entities.PremiumEntitlement.filter({ user_id: purchase.appUserId });
    const ent = existing && existing[0];
    if (isLifetimePurchase) {
      if (ent) {
        if (!ent.is_permanent) {
          await db.entities.PremiumEntitlement.update(ent.id, { is_permanent: true, plan_type: "lifetime", total_paid: 20 });
        }
        console.log("payments-webhook: upgraded to lifetime Pro", { userId: purchase.appUserId, purchaseId: purchase.id });
      } else {
        await db.entities.PremiumEntitlement.create({
        ...
        console.log("payments-webhook: granted lifetime Pro", { userId: purchase.appUserId, purchaseId: purchase.id });
      }
    } else if (isInstallmentPurchase) {
      const now = new Date();
      const base = ent?.pro_until ? new Date(ent.pro_until) : now;
      const anchor = base > now ? base : now;
      const newProUntil = new Date(anchor.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString();
      const newTotal = (ent?.total_paid || 0) + 5;
      const permanent = newTotal >= 20;
      if (ent) {
        await db.entities.PremiumEntitlement.update(ent.id, {
          total_paid: newTotal,
          pro_until: ent.is_permanent ? ent.pro_until ?? newProUntil : newProUntil,
          is_permanent: ent.is_permanent || permanent,
          plan_type: ent.is_permanent ? ent.plan_type : (permanent ? "lifetime" : ent.plan_type || "monthly"),
        });
        console.log("payments-webhook: installment Pro applied", { userId: purchase.appUserId, totalPaid: newTotal, permanent, purchaseId: purchase.id });
      } else {
        await db.entities.PremiumEntitlement.create({
          user_id: purchase.appUserId,
          purchase_id: purchase.id,
          ...
          plan_type: permanent ? "lifetime" : "monthly",
          total_paid: newTotal,
          pro_until: newProUntil,
        });
        console.log("payments-webhook: granted installment Pro", { userId: purchase.appUserId, totalPaid: newTotal, permanent, purchaseId: purchase.id });
      }
    } else {
      console.warn("payments-webhook: unknown productId, no grant applied", { productId: purchase.productId, purchaseId: purchase.id });
    }
  } else {
    // Anonymous buyer (no app user at checkout). Budgey requires login, so this shouldn't happen —
    // log it so it can be reconciled manually using the buyer's email.
    console.warn("payments-webhook: Pro purchase has no appUserId to grant to", { checkoutId, buyerEmail });
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any, eventType: string): Promise<Response> {
  // Canceled = ended early (before all 4 cycles → hasn't paid $20 → revoke).
  // Expired = completed all 4 billing cycles ($20 paid) → convert to permanent Premium.
  ...
  // ===== APP-SPECIFIC =====
  // CANCELED (ended early, hasn't paid $20): revoke Premium by deleting the entitlement — but
  // only if it is NOT permanent (a lifetime buyer's entitlement must survive a stray monthly
  // cancel). EXPIRED (completed all 4 cycles = $20 paid): convert to permanent instead of
  // revoking. Runs BEFORE the status flip so a throw leaves the status for retry. Idempotent.
  const isExpired = eventType === SUBSCRIPTION_EXPIRED;
  if (purchase.appUserId) {
    const existing = await db.entities.PremiumEntitlement.filter({ user_id: purchase.appUserId });
    for (const ent of existing || []) {
      if (isExpired) {
        // Completed all billing cycles — unlock Premium permanently.
        if (!ent.is_permanent) {
          await db.entities.PremiumEntitlement.update(ent.id, { is_permanent: true });
        }
        console.log("payments-webhook: monthly plan completed — Premium is now permanent", { userId: purchase.appUserId, purchaseId: purchase.id });
      } else if (!ent.is_permanent) {
        // Canceled before completing all cycles — revoke.
        await db.entities.PremiumEntitlement.delete(ent.id);
        console.log("payments-webhook: revoked Budgie Premium (canceled early)", { userId: purchase.appUserId, purchaseId: purchase.id });
      } else {
        // Already permanent (lifetime buyer) — leave it.
        console.log("payments-webhook: keeping permanent Premium entitlement", { userId: purchase.appUserId, purchaseId: purchase.id });
      }
    }
  } else {
    console.warn("payments-webhook: subscription event has no appUserId", { subscriptionId });
  }
  // ===== END APP-SPECIFIC =====
